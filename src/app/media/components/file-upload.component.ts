  ...
  template: `
    <div class="text-center upload-zone">
      <div ng2FileDrop
        ...
        <label class="custom-file">
          <input type="file" ng2FileSelect [uploader]="uploader" name="mediaPath"
           [multiple]="multiple" (change)="fileSelect()" class="custom-file-input" />
          <span class="custom-file-control"></span>
        </label>
      </div>
      <div class="progress" [hidden]="!progress || error">
      ...
      <p *ngIf="uploader.queue.length && !autoUpload">
        <button type="button" class="btn btn-primary" *ngIf="!uploadOnSelect"
         (click)="upload()">{{options.uploadText || 'Upload' | translate}}</button>
...
export class FileUploadComponent implements OnInit, AfterViewInit {
  ...
  constructor(private translate: TranslateService, private authService: AuthService, private mediaService: MediaService) {
  }
